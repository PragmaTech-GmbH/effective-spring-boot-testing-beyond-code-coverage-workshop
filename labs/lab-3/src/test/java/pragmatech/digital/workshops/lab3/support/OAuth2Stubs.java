package pragmatech.digital.workshops.lab3.support;
...
/**
 * Turns a {@link WireMockServer} into a fake OIDC issuer so lab 3 can skip the
 * Keycloak Testcontainer entirely. Dropping Keycloak is the main lever that
 * lets the whole test suite share a single Spring context and run classes in
 * parallel — booting an IdP container per context tears that apart.
 *
 * <p>After {@link #stubOpenIdConfiguration()} the server answers:
 * <ul>
 *   <li>{@code GET /realms/{realm}/.well-known/openid-configuration}</li>
 *   <li>{@code GET /realms/{realm}/protocol/openid-connect/certs} — JWKS
 *       containing the public half of a locally generated RSA key.</li>
 * </ul>
 * Tokens minted via {@link #signedJwt(String, String...)} are signed with the
 * matching private key, so Spring Security's {@code NimbusJwtDecoder} will
 * verify them after fetching the stubbed JWKS exactly once.
 */
public class OAuth2Stubs {
  ...
  /**
   * Mint a signed JWT for {@code subject} with the given space-separated
   * {@code scopes}. Token lives for 5 minutes and carries the issuer claim
   * that matches {@link #issuerUri()} so Spring Security accepts it.
   */
